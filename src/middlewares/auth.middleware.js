...
// jwt에서 꺼낸 userId를 바탕으로 DB에서 사용자 조회 해야하기 때문에 prisma client 가져오기
import { prisma } from '../utils/prisma/index.js';

// express의 의존성이 존재하지 않기 때문에 함수형으로 구현
export default async function (req, res, next) {
    try {
        const { authorization } = req.cookie; // req.cookie에 있는 authorization 쿠키 가져오기
        ...
        const [tokenType, token] = authorization.split(' '); // ' '를 기준으로 분리해서 배열로 만듬
        ...
        if (!user) {
            throw new Error('토근 사용자가 존재하지 않습니다.');
        }
